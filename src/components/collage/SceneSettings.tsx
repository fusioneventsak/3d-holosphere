...
const SceneSettings: React.FC = () => {
  ...
  // Calculate valid grid sizes based on aspect ratio
  const getValidGridSizes = () => {
    const validSizes = [];
    for (let i = 4; i <= 500; i++) {
      const width = Math.ceil(Math.sqrt(i * settings.gridAspectRatio));
      const height = Math.ceil(i / width);
      if (width * height === i) {
        validSizes.push(i);
      }
    }
            ...
            {settings.animationPattern === 'grid' ? (
              <div>
                ...
                <select
                  ...
                  className="w-full bg-black/30 border border-gray-700 rounded-md py-2 px-3 text-white"
                >
                  {getValidGridSizes().map(size => (
                    <option key={size} value={size}>
                      {size} photos ({Math.ceil(Math.sqrt(size * settings.gridAspectRatio))}x{Math.ceil(size / Math.ceil(Math.sqrt(size * settings.gridAspectRatio)))})
                    </option>
                  ))}
                </select>
              </div>
