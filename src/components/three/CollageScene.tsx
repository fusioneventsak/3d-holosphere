...
const PhotoPlane: React.FC<PhotoPlaneProps> = ({ url, position, rotation, pattern, speed, animationEnabled, size, settings, photos, index, wall }) => {
  ...
  const startDelay = useRef<number>(Math.random() * Math.PI);
  ...
  useEffect(() => {
    if (lastPattern.current !== pattern) {
      ...
      startDelay.current = Math.random() * Math.PI;
  ...
  useFrame((state, delta) => {
    ...
    if (transitionProgress.current < 1) {
      transitionProgress.current = Math.min(1, transitionProgress.current + delta);
    }
