...
const PhotosContainer: React.FC<{ photos: Photo[], settings: any }> = ({ photos, settings }) => {
  const photoProps = useMemo(() => {
    ...
    return photos.map((photo, index) => {
      ...
      const y = gridYOffset + ((gridHeight - 1 - row) * verticalSpacing) + 3; // Ensure minimum height above floor
      
      const position: [number, number, number] = [x, y, 2]; // All photos on same Z plane
